...
import {
  ...
  Settings, HelpCircle, Bot, ChevronsLeft, ChevronsRight,
} from 'lucide-react'
...
export function SessionSidebar({
  ...
  return (
    <aside className={`session-sidebar${collapsed ? ' session-sidebar-collapsed' : ''}`}>
      ...
      {/* 折叠时的图标列表（始终渲染，通过 CSS 控制显示） */}
      <div className="session-collapsed-actions">
        <button
          className="session-collapsed-new-btn"
        ...
        {sessions.map(session => (
          <button
            key={session.id}
            className={`session-collapsed-item${activeSessionId === session.id ? ' active' : ''}`}
